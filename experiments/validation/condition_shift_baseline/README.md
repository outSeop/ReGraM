...
baseline 조사와 코드 출처는 [baseline_survey.md](/Users/song-inseop/연구/ReGraM/experiments/validation/condition_shift_baseline/baseline_survey.md)에 정리한다.
...
현재 공통 summary schema는 `contracts.py`의 `condition_shift_baseline.summary.v1`을 기준으로 둔다.
...
1. baseline 이름을 정한다.
2. baseline 후보와 코드 출처를 `baseline_survey.md`와 `configs/baseline_registry.yaml`에서 확인한다.
